...
import LoadingSpinner from "@/app/(auth)/components/LoadingSpinner";
import Typewriter from "@/app/(auth)/components/Typewriter";
import Link from 'next/link'; // Import Link

function LoginFormContent({ errorMessage }: { errorMessage: string | undefined }) {
      ...
      <>
        ...
        <div className={"LoginForm flex flex-col gap-4 animate-fade-in animation-delay-400"}>
          ...
          <div className={"flex items-center justify-center"}>
            <Link href="/request-account" legacyBehavior>
              <a className="group flex items-center space-x-2 text-sm text-slate-300 cursor-pointer hover:text-white transition-all duration-300 bg-white/5 hover:bg-white/10 px-4 py-2 rounded-lg border border-white/10 hover:border-white/20 hover:shadow-md">
...
export default function LoginPage() {
  ...
  return (
      <>
        <AnimatedBackground />
        <form action={dispatch} className="LoginContainer flex flex-col w-full max-w-md gap-8 relative z-10 backdrop-blur-xl border border-white/20 rounded-2xl p-8 shadow-2xl bg-white/10">
          ...
          <LoginFormContent errorMessage={errorMessage} />
